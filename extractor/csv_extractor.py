...
class CsvGcsExtractor(Extractor):
    # Config keys
    FILE_LOCATION = 'file_location'

    """
    An Extractor that extracts records from GCS via CSV.
    """
    ...
    def _load_csv(self) -> None:
        ...
        if not hasattr(self, 'results'):
            ...
            for file in fs.ls(self.file_location):
                if file.split(".")[1] == "csv":
                    self.filenames.append(file.split("/")[-1].split(".")[0])
                    df = pd.read_csv(f'gs://{file}')
                    self.data_types.append(df.dtypes)
                    with fs.open(file, 'r') as fin:
                        lines =  fin.read().split("\n")[:-1]
                        self.results.append([x.split(",") for x in lines])

        results = self._serializing_csv()
        self.iter = iter(results)

    def _serializing_csv(self) -> TableMetadata:
        """
        Serialize a CSV without a presented model.
        """
        for result_index, result in enumerate(self.results):
            cols: List[ColumnMetadata] = []
            column_dict = result[0]
            for i in range(0, len(column_dict)):
                column = ColumnMetadata(
                    name=column_dict[i],
                    description="",
                    col_type=self.data_types[result_index][i].name,
                    sort_order=i+1)
                cols.append(column)
            yield TableMetadata(
                database="GCS",
                cluster=self.file_location,
                schema=self.file_location,
                name=self.filenames[result_index],
                description="",
                columns=cols,
                is_view=False)
